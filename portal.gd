...
func _ready() -> void:
	...
	for i in range(10):
		extra_meshes.append(portal_entry.duplicate())
		...
		if i>=5:
			...
			extra_meshes[i].get_mesh().surface_get_material(0).albedo_color = Color("white")
			extra_meshes[i].set_layer_mask_value(10, false)
			extra_meshes[i].set_layer_mask_value(1, true)
	
	# Back face
	extra_meshes[0].global_position += portal_entry.global_basis.z.normalized()*thickness/2
	extra_meshes[5].global_position += portal_entry.global_basis.z.normalized()*thickness/2
	# Right
	extra_meshes[1].global_position -= portal_entry.global_basis.x.normalized()*size_x/2
	extra_meshes[1].rotate(extra_meshes[1].global_basis.y, deg_to_rad(-90))
	extra_meshes[1].get_mesh().size.x = thickness
	extra_meshes[6].global_position -= portal_entry.global_basis.x.normalized()*size_x/2
	extra_meshes[6].rotate(extra_meshes[6].global_basis.y, deg_to_rad(-90))
	extra_meshes[6].get_mesh().size.x = thickness
	# Left
	extra_meshes[2].global_position += portal_entry.global_basis.x.normalized()*size_x/2
	extra_meshes[2].rotate(extra_meshes[2].global_basis.y, deg_to_rad(90))
	extra_meshes[2].get_mesh().size.x = thickness
	extra_meshes[7].global_position += portal_entry.global_basis.x.normalized()*size_x/2
	extra_meshes[7].rotate(extra_meshes[7].global_basis.y, deg_to_rad(90))
	extra_meshes[7].get_mesh().size.x = thickness
	# Top face
	extra_meshes[3].global_position += portal_entry.global_basis.y.normalized()*size_y/2
	extra_meshes[3].rotate(extra_meshes[3].global_basis.x, deg_to_rad(-90))
	extra_meshes[3].get_mesh().size.y = thickness
	extra_meshes[8].global_position += portal_entry.global_basis.y.normalized()*size_y/2
	extra_meshes[8].rotate(extra_meshes[8].global_basis.x, deg_to_rad(-90))
	extra_meshes[8].get_mesh().size.y = thickness
	# Bottom face
	extra_meshes[4].global_position -= portal_entry.global_basis.y.normalized()*size_y/2
	extra_meshes[4].rotate(extra_meshes[4].global_basis.x, deg_to_rad(90))
	extra_meshes[4].get_mesh().size.y = thickness
	extra_meshes[9].global_position -= portal_entry.global_basis.y.normalized()*size_y/2
	extra_meshes[9].rotate(extra_meshes[9].global_basis.x, deg_to_rad(90))
